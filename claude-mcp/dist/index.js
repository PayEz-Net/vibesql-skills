...
// Database tools
// ---------------------------------------------------------------------------
server.tool('query', 'Execute a SQL query. For DDL operations (CREATE TABLE, ALTER TABLE, DROP TABLE), prefer the dedicated tools which include safety checks. This tool is unrestricted but will warn on DDL statements.', {
    ...
    try {
        ...
        return {
            content: [{ type: 'text', text: (text || '(no rows returned)') + warning }],
        };
    }
    catch (error) {
        return {
...
server.tool('describe_table', 'Get full table info: columns, constraints, indexes, foreign keys, and row count. Use this before modifying any table.', {
...
server.tool('alter_table', 'Alter a table (add column, add constraint, etc). Classifies the change by risk level before executing. Destructive changes (drop column, change type, rename) require confirm=true.', {
    ...
    try {
        ...
        if (classification.level === 'prohibited') {
            return {
                content: [{ type: 'text', text: `BLOCKED [${classification.code}]: ${classification.reason}. This operation is prohibited. Use explicit per-table/per-column operations instead.` }],
                isError: true,
            };
        }
            ...
            try {
                ...
                if (rowCount > 0) {
                    ...
                    if (deps.rows.length > 0) {
                        context += `\nFK dependents: ${deps.rows.map(r => r[0]).join(', ')}`;
                    }
                }
            }
            catch { /* table info unavailable */ }
            return {
                content: [{ type: 'text', text: `⚠ DESTRUCTIVE CHANGE [${classification.code}]: ${classification.reason}${context}\n\nTo proceed, call alter_table again with confirm=true. Show this warning to the user first.` }],
                isError: true,
            };
        }
...
server.tool('drop_table', 'Drop a table. Shows row count and FK dependents before dropping. Requires confirm=true — never auto-confirm. Always show the impact to the user first.', {
    ...
    try {
        ...
        const depList = dependents.rows.map(r => `  - ${r[0]}.${r[1]} (FK: ${r[2]})`).join('\n');
        const impact = [
            `Table: ${table}`,
        ...
        return {
            content: [{ type: 'text', text: `Table "${table}" dropped. ${rowCount > 0 ? `${rowCount} rows removed.` : ''}` }],
        };
    }
    catch (error) {
        return {
...
server.tool('analyze_change', 'Pre-flight analysis of a DDL statement. Shows what would change, the risk level (Sentinel code), and data impact — WITHOUT executing anything. Use this before any schema modification.', {
    ...
    try {
        ...
        if (nameMatch) {
            ...
            try {
                ...
                const deps = await client.getDependents(tableName);
                if (deps.rows.length > 0) {
                    sections.push(`**FK dependents:** ${deps.rows.map(r => r[0]).join(', ')}`);
                }
            }
            catch {
